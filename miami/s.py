...
if __name__ == "__main__":
    ...
    for comp in compounds:
        print(f"\n--- COMPOUND: {comp} ---")
        try:
            df_laps = f1_fe.get_laps_on_compound(
                YEAR, GP, TARGET_SESSION, compound=comp
            )
    ...
    try:
        df_speed_var = f1_fe.get_speed_trap_variance(
            YEAR, GP, TARGET_SESSION, min_laps=5
        )
    ...
    for comp in compounds:
        print(f"\n--- LONG RUNS ON {comp} ---")
        try:
            df_longrun_deg_rate = f1_fe.get_longrun_deg_rate(
                YEAR, GP, TARGET_SESSION, compound=comp
            )
            if df_longrun_deg_rate is not None and len(df_longrun_deg_rate) > 0:
                ...
                df_consistency = f1_fe.get_longrun_consistency(
                    YEAR, GP, TARGET_SESSION, compound=comp, min_laps=5
                )
                print(f"Consistency (Std Dev):")
                print(df_consistency.to_string(index=False))

                df_fuel_pace = f1_fe.get_fuel_corrected_pace(
                    YEAR, GP, TARGET_SESSION, compound=comp, window_size=5
                )
                print(f"Fuel Corrected Base Pace:")
                print(df_fuel_pace.to_string(index=False))

                df_deg_total = f1_fe.get_longrun_deg_total(
                    YEAR,
                    GP,
                    TARGET_SESSION,
                    compound=comp,
                    min_laps=5,
                    expected_stint_length=18,
                )
